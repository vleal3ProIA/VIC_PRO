...
class MagicLinkSentPage extends ConsumerStatefulWidget {
  const MagicLinkSentPage({required this.email, super.key});
...
class _MagicLinkSentPageState extends ConsumerState<MagicLinkSentPage> {
  ...
  Future<void> _resend() async {
    ...
    final result = await repo.signInWithMagicLink(widget.email);
    if (!mounted) return;
    result.match(
      (failure) =>
          context.showSnack(authFailureMessage(context, failure), isError: true),
      (_) => context.showSnack(context.l10n.magicLinkSentTitle),
    );
