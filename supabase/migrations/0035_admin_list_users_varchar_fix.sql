-- ============================================================================
-- 0035 · Fix admin_list_users: cast varchar -> text para `auth.users.email`
-- ----------------------------------------------------------------------------
-- Segunda iteracion del fix de /admin/users. Tras 0034 (que arreglo el
-- error 22P02 del enum subscription_status) aparece ahora un 42804
-- (datatype_mismatch).
--
-- Causa: `auth.users.email` es de tipo `varchar(255)` en Supabase Postgres,
-- pero el composite type `public.admin_user_row.email` declara `text`.
-- Postgres NO castea implicitamente varchar -> text dentro de un row type
-- (aunque si lo hace en contextos escalares). Resultado: la funcion
-- compila pero falla al ejecutarse con "structure of query does not match
-- function result type".
--
-- Fix: castear `u.email::text` explicitamente en el SELECT.
--
-- Aprovecho para castear tambien `u.email_confirmed_at` etc. solo en
-- caso de que Supabase los cambie en una version futura — no cuesta nada
-- y nos asegura robustez ante upgrades del esquema auth.
-- ============================================================================
...
begin
  ...
  with active_sub as (
    select distinct on (tm.user_id)
      ...
      p.slug as plan_slug,
      p.name as plan_name,
      ts.status::text as sub_status,  -- enum -> text (fix 0034)
  ...
  base as (
    select
      u.id,
      u.email::text                              as email,           -- varchar(255) -> text
