...
| Super admin + capabilities (A2) | Migración 0045 (`super_admin_promote_by_email.sql`): nueva RPC `super_admin_promote_to_admin_by_email(text)` que resuelve email → user_id y delega en `super_admin_promote_to_admin(uuid)` (de 0044). Sin Edge Functions nuevas, sin secrets. **Cambios Flutter** (sin acción manual, solo `flutter build web` + redeploy del bundle): nueva ruta `/admin/admins` SOLO super admin (gestión completa: lista admins + capability toggles + promote por email + revoke), filtrado de cards en `/admin` según las capabilities del user (admins normales ven sólo lo que el super les ha activado; super ve TODO + sección "Super admin tools"), guards 2c (super-only) y 2d (capability-gated) en el router. Defensa en profundidad triple: UI filtra → router guard bloquea → RPCs server-side re-validan `is_super_admin()` / `has_capability()`. **Acción super admin tras desplegar**: entra a `/admin/admins`, promueve cuentas a admin con `Promote user to admin` (email), y activa/desactiva capabilities por admin con los switches de cada card. |
| Super admin + capabilities (A3) | Capability gate server-side en Edge Functions. Nuevo helper `_shared/capability.ts`. **Re-desplegar 7 Edge Functions** (sin migración nueva, sin secrets): `supabase functions deploy admin-users admin-coupons admin-plans admin-plans-prices admin-stripe-branding broadcast-dispatch run-audit`. Cada una ahora exige, además de `role='admin'`, la capability concreta vía `has_capability()`: admin-users→manage_users, admin-coupons→manage_coupons, admin-plans + admin-plans-prices→manage_plans, admin-stripe-branding→manage_branding, broadcast-dispatch→manage_broadcasts, run-audit→run_audits. El super admin pasa siempre. **Acción admin**: solo el redeploy de las EFs; nada en BD (la RPC `has_capability` ya existe desde 0044). |
| Super admin + capabilities (A4) | Migración 0048 (`capability_rls_gates.sql`). Cierra el gap residual de A3: las 4 capabilities sin EF (`manage_flags`/`manage_changelog`/`manage_incidents`/`manage_app_branding`) escribían directo a tabla protegidas solo por RLS `is_admin()`. Migra las policies de escritura de `feature_flags`, `feature_flag_overrides`, `changelog_entries`, `incidents`, `app_branding` de `is_admin()` → `has_capability('<cap>')`. **Reads públicos preservados** (flags, app_branding); drafts de changelog/incidents gateados por capability; setup wizard intacto. **Acción admin**: `supabase db push` (solo aplica 0048). Sin Edge Functions, sin secrets. **Validación post-deploy**: como super, /admin/flags, /admin/changelog, /admin/incidents, /admin/app-branding deben seguir funcionando (guardar cambios OK). Un admin sin la cap no puede escribir ni vía REST directo. |
| Security hardening (web + supply chain + audit) | **(1)** `web/.htaccess` CSP: añadido Sentry a `connect-src` + `frame-ancestors 'self'`. **(2)** `web/index.html`: SRI (`integrity` + `crossorigin`) en el `<script>` de unpkg WebAuthn. **(3)** Nuevo audit check `access.orphan_admins` en `run-audit`. **Acción admin**: `flutter build web` + redeploy del bundle (recoge .htaccess + index.html) — automático vía deploy.yml al mergear a main. Re-desplegar la EF del audit: `supabase functions deploy run-audit`. Sin migración, sin secrets. **Acción dashboard (1 clic, opcional pero recomendado)**: Supabase → Authentication → Settings → activar **"Leaked password protection"** (chequea passwords contra HaveIBeenPwned). No es togglable por CLI. **Si subes la versión del paquete WebAuthn en index.html**: regenera el hash SRI con `curl -sSL <url> \| openssl dgst -sha384 -binary \| openssl base64 -A` o WebAuthn dejará de cargar. |
| Leaked password protection (free, client-side) | El toggle de Supabase es Pro-only; lo implementamos en código con la API Pwned Passwords (k-anonimato). **Acción admin**: solo `flutter build web` + redeploy (automático vía deploy.yml al mergear). Sin migración, sin EF, sin secrets. `web/.htaccess` añade `api.pwnedpasswords.com` a CSP `connect-src` (ya incluido). Fail-open ante caída de HIBP. **Recomendado en dashboard (free, 1 clic c/u)**: Authentication → Sign In/Providers → Email: subir **Minimum password length a 8** + **Password requirements a "Lowercase, uppercase letters, digits and symbols"** (enforca server-side la misma política que valida el cliente; cierra el bypass por API directa de signup). |
| Auto-rescan de uploads stuck | Nueva task `rescan_stuck_uploads` en la Edge Function `maintenance-cron` + 4º job en `.github/workflows/maintenance.yml` (diario 04:00 UTC). Reintenta automáticamente los uploads con scan VirusTotal en `error` > 24h → el finding `uploads.scan_errors` del Audit Center converge a 0 sin acción manual. **Acción admin**: `supabase functions deploy maintenance-cron`. El workflow se actualiza solo al mergear. Sin migración, sin secrets nuevos (reusa `CRON_SECRET`). Para limpiar el backlog YA sin esperar al cron: GitHub → Actions → Maintenance → Run workflow → `rescan_stuck_uploads`. |
