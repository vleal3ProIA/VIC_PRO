...
| Webhook secret rotate | Re-desplegar Edge Function `webhook-dispatch` (`supabase functions deploy webhook-dispatch`) — añade `action: "rotate_secret"`. Re-desplegar `verify-password` (`supabase functions deploy verify-password`) — añade `webhook_secret_rotate` al whitelist de action_kinds. **Sin migraciones, sin secrets nuevos**. UI nueva en `/account-settings/webhooks/:id` con botón "Rotate signing secret" que primero pide password via `ReauthDialog` y luego muestra el nuevo HMAC secret una sola vez en un dialog copiable. El secret viejo deja de funcionar inmediatamente — el cliente debe actualizar su receptor antes del próximo dispatch. Cierra el último TODO server-side de re-auth (PR-F). |
